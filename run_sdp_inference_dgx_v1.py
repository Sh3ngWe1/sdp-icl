...
# ================= 1. 實驗設定區 =================
# 🚀 SDP-ICL 核心配置 Qwen Sanitized QA
# EXPERIMENT_CONFIGS = [
#     {
#         "task_name": "sdp_qa_qwen_n100",
#         "model_type": "qwen",
...
# 🚀 SDP-ICL 核心配置 Qwen Standard QA
EXPERIMENT_CONFIGS = [
    {
        "task_name": "sdp_qa_qwen_standard_n100",
        "model_type": "qwen",
        "exemplar_db_path": "data/synthetic/squad_synthetic_train.json", # 👈 注意這裡是 synthetic (包含機密毒藥的)
        "test_set_path": "data/qa_validation_set.json",
        "description": "⚖️ SDP 對照組: Qwen-2.5-7B (N=100, Standard 極限)"
    }
]

# 🚀 SDP-ICL 核心配置 Llama Sanitized QA
# EXPERIMENT_CONFIGS = [
#     {
#         "task_name": "sdp_qa_llama_sanitized_n100",
#         "model_type": "llama",
#         "exemplar_db_path": "data/sanitized/squad_sanitized_train.json",
#         "test_set_path": "data/qa_validation_set.json",
...
# 🚀 SDP-ICL 核心配置 Llama Standard QA
# EXPERIMENT_CONFIGS = [
#     {
#         "task_name": "sdp_qa_llama_standard_n100",
#         "model_type": "llama",
#         "exemplar_db_path": "data/synthetic/squad_synthetic_train.json",
#         "test_set_path": "data/qa_validation_set.json",
#         "description": "⚖️ SDP 對照組: Llama-3.1-8B (N=100, Standard 極限)"
#     }
# ]
...
API_KEY = "lm-studio"
LLMSTER_MODEL_NAME = "qwen2.5-7b-instruct" # ⚠️ 請確保這與你在 llmster 載入的模型名稱完全一致
# LLMSTER_MODEL_NAME = "meta-llama-3.1-8b-instruct"
TARGET_CONTEXT_LENGTH = 8192  # 🎯 設定 8K context window（避免 token limit 錯誤）

random.seed(42)

# 設定 logging，同時輸出到終端機和檔案
log_dir = "logs"
if not os.path.exists(log_dir):
    os.makedirs(log_dir)

config = EXPERIMENT_CONFIGS[0]
log_file = os.path.join(log_dir, f"{config['task_name']}.log")
...
# 將 handlers 加到 logger
logger.addHandler(console_handler)
logger.addHandler(file_handler)

logging.info(f"Log 檔案建立於: {log_file}")
...
async def generate_with_api(prompt, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            response = await client.chat.completions.create(
                model=LLMSTER_MODEL_NAME,
                messages=[
                    {"role": "system", "content": "You are Qwen, created by Alibaba Cloud. You are a precise answer extraction system."},
...
async def single_inference_task(semaphore, n, question, context, exemplar_db, model_type):
    ...
    async with semaphore:  # 限制並發數量
        ...
        try:
            ...
            # 3. 呼叫 API
            answer = await generate_with_api(prompt)
...
async def run_single_experiment(config):
    ...
    output_path = f"results/{task_name}_results.jsonl"  # 改用 .jsonl 格式
...
async def main():
    ...
    # 1. 載入 8K context 模型
    logging.info("\n🚀 開始 SDP-ICL 推論實驗")
    instance_id = load_model_8k()
    
    if not instance_id:
        logging.error("❌ 模型載入失敗，程式終止")
        return
    ...
    try:
        ...
        # 🆕 3. 生成錯誤報告
        generate_error_report(all_error_stats)
...
def generate_error_report(all_error_stats):
    """生成錯誤統計報告並儲存到 JSON 檔案"""
    ...
    # 儲存到 JSON 檔案
    error_report_path = f"results/error_report_{time.strftime('%Y%m%d_%H%M%S')}.json"
